...
namespace Presentation.BisleriumBlog.Controllers
{
    ...
    public class CommentController : Controller
    {
        ...
        [Authorize(Roles = "Blogger")]
        [HttpGet("history")]
        public async Task<IActionResult> GetBlogCommentHistory()
        {
            try
            {
                var token = HttpContext.Request.Headers["Authorization"].FirstOrDefault()?.Split(" ").Last();
                ...
                var history = await _blogCommentService.GetBlogCommentHistoryByUserID(userId);
            ...
            {
                _logger.LogError(ex, "An error occurred while retrieving the comment history.");
                return StatusCode(500, "An error occurred while retrieving the comment history. Please try again.");
            }
        }
